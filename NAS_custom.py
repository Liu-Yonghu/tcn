...
from keras_tuner_local import backend
from keras_tuner_local import errors
from keras_tuner_local import utils
from keras_tuner_local.engine import base_tuner
from keras_tuner_local.engine import tuner_utils
...
X_train, Y_train, X_val, Y_val, X_test, Y_test= load_dataset(parent_dir,"preprocessed-training-Osama.csv","preprocessed-validation-Osama.csv","preprocessed-testing-Osama.csv")
...
class MyHyperModel(keras_tuner_local.HyperModel):
    def build(self, hp):
        # Supponiamo che mlp_model sia la tua MLP già allenata e tcn_model sia la TCN
        x = Input(shape=(15, 4))
        tcn_model=model_TCN(hp.Choice("hidden", [2,3,4]),hp.Choice("nb_filters", [8,16,32]),hp.Choice("k_size", [2,3,4,5]),hp.Choice("dense", [8,16,32]))
    ...
    def fit(self, hp, model,trial,execution, train_ds, val_ds, test_ds, path_save, callbacks=None, **kwargs):
        ...
        plt.legend()
        plt.savefig('./autokeras_mlp4tcn_res_exp2_all/trial_'+str(trial.trial_id)+'/learning_curve_mlptcn'+str(execution)+'.pdf')
        plt.close()
        ...
        try:
            model.load_weights("./autokeras_mlp4tcn_res_exp2_all/trial_"+str(trial.trial_id)+"/ckpt_exec"+str(int(execution))+".h5")
            ...
            plt.tight_layout()
            plt.savefig('./autokeras_mlp4tcn_res_exp2_all/trial_'+str(trial.trial_id)+'/test_results_'+str(execution)+'.pdf')
            plt.close()
            # #################################
            ...
            plt.scatter(predictions_train[:,0], predictions_train[:,1], color='red', label='Distanze predette', alpha=0.5)
            plt.xlabel('Distanza y')
            plt.ylabel('Distanza x')
            plt.xlim(0,3)
            plt.ylim(0,3)
            ...
            plt.tight_layout()
            plt.savefig('./autokeras_mlp4tcn_res_exp2_all/trial_'+str(trial.trial_id)+'/train_results_'+str(execution)+'.pdf')
            plt.close()
            ...
            plt.tight_layout()
            plt.savefig('./autokeras_mlp4tcn_res_exp2_all/trial_'+str(trial.trial_id)+'/val_results_'+str(execution)+'.pdf')
            plt.close()
        ...
        for callback in callbacks:
        #The "my_metric" is the objective passed to the tuner.
            callback.on_train_end(logs={"best_val_loss": best_val_loss,"best_test_loss": best_test_loss,"best_epoch": best_epoch })
        
        return best_test_loss

# class  BayesianOptimization(keras_tuner_local.BayesianOptimization):
class GridSearchTuner(keras_tuner_local.GridSearch):
    ...
    def _deepcopy_callbacks(self, callbacks):
        try:
            ...
            raise keras_tuner_local.errors.FatalValueError(
    ...
    def _configure_tensorboard_dir(self, callbacks, trial, execution):
        for callback in callbacks:
            if callback.__class__.__name__ == "TensorBoard":
                ...
                hparams = keras_tuner_local.engine.tuner_utils.convert_hyperparams_to_hparams(
                    trial.hyperparameters
                )
                callbacks.append(
...
tuner = GridSearchTuner(
    MyHyperModel(),
    objective=keras_tuner_local.Objective("test_loss", "min"),
    max_trials=80,
